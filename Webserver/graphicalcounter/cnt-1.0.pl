...
#
   ...
   #ASHLEYCHANGEDTHIS (from linux to a more windoze friendly pathname
   $counterFile = "$ENV{'DOCUMENT_ROOT'}\\cntlog";
...
sub incrementTotalReads {
  if (-e $counterFile) {
    ...
    #flock(COUNT,2); File Locking is unimplemented in windows...
    seek(COUNT,0,0);
  }
...
#   }
#  else {
#    die("Counter $ENV{'DOCUMENT_ROOT'}$countedPage does not exist.\n");
#  }

  open(COUNT,">$counterFile") || die "$0: can\'t open $counterFile: $!\n";
  #flock(COUNT,2); unimplemented on windows
  seek(COUNT,0,0);
